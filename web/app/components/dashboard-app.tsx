  ...
  return (
    <div className="grid gap-4">
      ...
      <DataPanel title="Portfolio Risk Engine · Shadow mode">
        ...
        <div className="mt-4 overflow-x-auto"><table className="w-full min-w-[720px] text-left text-sm"><thead className="text-xs text-slate-400"><tr><th className="py-2">Symbol</th><th>Phía / quantity</th><th>Notional</th><th>Tỷ trọng</th><th>Open risk</th><th>Stop Loss</th></tr></thead><tbody>{(portfolioRisk?.portfolio.positions ?? []).map((item) => <tr className="border-t border-white/10" key={`${item.symbol}-${item.side}`}><td className="py-3 font-black">{item.symbol}</td><td>{item.side} · {number(item.quantity)}</td><td>{money(item.notional)}</td><td>{percent(item.notional_fraction * 100)}</td><td>{item.open_risk === null ? "Không xác minh" : money(item.open_risk)}</td><td className={item.protected ? "text-emerald-300" : "text-red-300"}>{item.stop_loss ?? "Thiếu"}</td></tr>)}</tbody></table></div>
        <div className="mt-4 rounded border border-cyan-400/15 bg-cyan-950/10 p-3">
          <div className="flex flex-wrap items-center justify-between gap-2"><b>Tương quan danh mục · Shadow only</b><span className={portfolioRisk?.portfolio.correlation.status === "COMPLETE" ? "text-emerald-300" : "text-amber-300"}>{portfolioRisk?.portfolio.correlation.status === "COMPLETE" ? "Đủ coverage" : "Chưa đủ coverage"}</span></div>
          <div className="mt-3 grid gap-3 md:grid-cols-3"><Metric label="Exposure điều chỉnh" value={`${money(portfolioRisk?.portfolio.correlation.adjusted_exposure)} · ${percent((portfolioRisk?.portfolio.correlation.adjusted_exposure_fraction ?? 0) * 100)}`} /><Metric label="Cửa sổ nến đóng" value={`${portfolioRisk?.portfolio.correlation.lookback ?? 0} × ${portfolioRisk?.portfolio.correlation.interval ?? "-"}`} /><Metric label="Ngưỡng cluster" value={number(portfolioRisk?.portfolio.correlation.threshold ?? 0)} /></div>
          {(portfolioRisk?.portfolio.correlation.clusters.length ?? 0) > 0 && <div className="mt-3 text-sm text-cyan-200">Cụm tương quan: {portfolioRisk?.portfolio.correlation.clusters.map((cluster) => `${cluster.symbols.join(" + ")} (${percent(cluster.notional_fraction * 100)})`).join("; ")}</div>}
          {(portfolioRisk?.portfolio.correlation.pairs.length ?? 0) > 0 && <div className="mt-2 text-xs text-slate-400">{portfolioRisk?.portfolio.correlation.pairs.map((pair) => `${pair.symbol_a}/${pair.symbol_b}: ${number(pair.correlation)}${pair.same_direction ? " cùng chiều" : " ngược chiều"}`).join(" · ")}</div>}
          {(portfolioRisk?.portfolio.correlation.reasons.length ?? 0) > 0 && <p className="mt-2 text-sm text-amber-300">{portfolioRisk?.portfolio.correlation.reasons.join("; ")}</p>}
        </div>
      </DataPanel>
      <DataPanel title="Audit quyết định Shadow">
        <p className="mb-3 text-sm text-slate-400">Snapshot chỉ lưu khi trạng thái an toàn thay đổi; fingerprint bỏ qua nhiễu giá/equity nhỏ nhưng vẫn giữ vị thế, quantity, Stop Loss, giới hạn và quyết định.</p>
        <div className="mb-4 grid gap-2 sm:grid-cols-3"><Metric label="Snapshot trạng thái" value={number(portfolioRisk?.audit_summary.snapshots ?? 0)} /><Metric label="WOULD ALLOW" value={number(portfolioRisk?.audit_summary.by_decision.WOULD_ALLOW ?? 0)} /><Metric label="WOULD REJECT" value={number(portfolioRisk?.audit_summary.by_decision.WOULD_REJECT ?? 0)} /></div>
        <div className="grid gap-2">{(portfolioRisk?.audits ?? []).slice(0, 15).map((audit) => <div className="rounded border border-white/10 bg-white/[0.02] p-3" key={audit.audit_id}><div className="flex flex-wrap items-center justify-between gap-2"><b>{audit.event === "PRE_TRADE" ? `${audit.symbol} · ${audit.side}` : "Snapshot danh mục"}</b><span className={audit.decision === "WOULD_REJECT" ? "text-red-300" : audit.decision === "WOULD_ALLOW" ? "text-emerald-300" : "text-slate-400"}>{audit.decision}</span></div><p className="mt-1 text-xs text-slate-500">{new Date(audit.created_at).toLocaleString("vi-VN")} · {audit.fingerprint.slice(0, 16)}</p>{audit.reasons.length > 0 && <p className="mt-2 text-sm text-amber-300">{audit.reasons.join("; ")}</p>}</div>)}{(portfolioRisk?.audits.length ?? 0) === 0 && <p className="text-sm text-slate-500">Chưa có audit snapshot.</p>}</div>
      </DataPanel>
